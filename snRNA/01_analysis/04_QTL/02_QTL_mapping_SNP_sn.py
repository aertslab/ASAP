...
output_dirs = [
    'sn/parquets/snp', 'sn/results/snp',
    'sn/parquets/indel', 'sn/results/indel',
...
for file in all_files:
    ...
    covariates_file = f'/sn/covariates/snp/{name}_cov.txt'
    prefix = f'sn/parquets/snp/{name}'
    output_file = f'sn/results/snp/eQTL_{name}.csv.gz'
...
# indels eQTL analysis no PCA
for file in all_files:
    print(f'Now running for {file}')
    name = extract_name(file, suffix)
    print(name)
    expression_bed = os.path.join(bed_path, file)
    covariates_file = f'/sn/covariates/indel/{name}_cov.txt'
    prefix = f'sn/parquets/indel/{name}'
    output_file = f'sn/results/indel/eQTL_{name}.csv.gz'
    print(f'Running for {name}')
    try:
        run_eqtl_analysis(plink_prefix_path, expression_bed, covariates_file, prefix, output_file)
    except Exception as e:
