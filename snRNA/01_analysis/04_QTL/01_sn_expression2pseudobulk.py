...
for cell_type in cell_types:
    ...
    print(f"Number of unique donors: {pdata[pdata.obs['cell_type_simplified'] == cell_type].obs['vireo_assignment'].nunique()}")

    if pdata[pdata.obs['cell_type_simplified'] == cell_type].obs['vireo_assignment'].nunique() < 30:
        print(f"Skipping {cell_type} because of low number of donors")
        continue
   
    cell_type_data = pdata[pdata.obs['cell_type_simplified'] == cell_type]
    cell_type_data.layers["counts"] = cell_type_data.X.copy()
    sc.pp.normalize_total(cell_type_data, target_sum=1e6) 
    ...
    output_path = f'sn/phenotypes/{date}_sn_{cell_type_out}_log1p_norm.bed.gz'
    ...
    cov_snp.to_csv(f'sn/covariates/snp/{date}_sn_{cell_type_out}_cov.txt',sep='\t')

    cov_indel = pd.concat([cov, indel_donor_pca_df], axis=0)
    cov_indel.to_csv(f'sn/covariates/indel/{date}_sn_{cell_type_out}_cov.txt',sep='\t')
    ...
    cov_snp = pd.concat([cov, snps_donor_pca_df], axis=0)
    cov_snp.to_csv(f'sn/covariates/snp/{date}_sn_{cell_type_out}_pca_cov.txt',sep='\t')

    cov_indel = pd.concat([cov, indel_donor_pca_df], axis=0)
    cov_indel.to_csv(f'sn/covariates/indel/{date}_sn_{cell_type_out}_pca_cov.txt',sep='\t')
    ...
    cov = cov.T
    cov.columns = cov.columns.str.replace(f'_{cell_type}', "", regex=False)
    cov.to_csv(f'sn/covariates/{date}_sn_{cell_type_out}_noexpr_cov_plotting.txt',sep='\t')
    print(f"Saved {cell_type} covariates to covariates/{date}_{cell_type_out}_sn_cov.txt")
